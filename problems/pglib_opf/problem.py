...
independent verifier accepts at 1e-6 pu whose cost is below the listed value by more than the table's rounding.
"""
...
MAXIMIZE = False
FAIL_SCORE = -1.0
WIN_MARGIN = 1e-4  # BASELINE.md has 5 significant figures: half a unit in the last place is at most 5e-5 relative
...
def beats(v, rec):
    return rec is None or v < rec * (1 - WIN_MARGIN)
...
and being feasible to 1e-6 on every case, every run.
...
INTERFACE CONTRACT (keep exactly):
  ...
  the checker enforces, at 1e-6 pu / degrees: voltage bounds, P/Q limits (zero for status-0 gens), nodal P and Q
